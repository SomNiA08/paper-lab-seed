...
격리했다. `~/.claude/` 원본은 제거 완료 — 2026-07-19 실측: commands/skills/agents 18파일 전부 홈에 부재 확인(이중 존재 해소됨).

> ⛔ **이 리포는 실행하지 마라 — 보관소다.** 훅·`SOUL/AGENTS/CLAUDE.md`·`MODELS.md`·`settings.json`이
> 없다(= append-only 벽·회고 가드 등 안전장치 0, 판정 모델 배치 원천 없음). 이 씨앗을 그대로 Claude Code로
> 열어 커맨드를 돌리지 마라. **착공(paper-lab 스폰) 시에만** 킷(`harness-kit`) 최신본에서 스폰하며, 그때
> 훅 5종·`MODELS.md`·에이전트 `model:` 프론트매터를 함께 갖춘다(킷 `00_사용법.md` 이식 절차). 여기 에이전트
> 10종은 RFP용이라 논문용으로 **재설계 대상** — 지금 이 씨앗에 프론트매터·훅을 다는 것은 버려질 자산에 대한 낭비다.
